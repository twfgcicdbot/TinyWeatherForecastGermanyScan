            ...
            for apk_file_tmp in apk_files:
                try:
                    ...
                    try:
                        ...
                        index_html_soup.select('script[type="application/ld+json"]')[
                            0
                        ].string = schema_org_meta
                        ...
                        try:
                            if (
                                len(
                                    list(
                                        index_html_soup.select(
                                            'body script[src*="gitlab"]'
                                        )
                                    )
                                )
                                > 0
                            ):
                                for script_tag in index_html_soup.select(
                                    'body script[src*="gitlab"]'
                                ):
                        ...
                        try:
                            css_links = list(
                                index_html_soup.select('head link[rel="stylesheet"]')
                            )
    ...
    <script>
        try {
            ...
            console.log("ERROR: failed to make level 3 toc entries toggle visibility of children items on click -> error: " + e);
                            ...
                            index_html_soup.select("#page-footer-text")[0].insert_after(
                                toc_js_soup
                            )
                        except Exception as error_msg:
                            logging.error(
                                "failed to add ToC JavaScript code -> error: "
                                + str(error_msg)
                            )

                        try:
                            if (
                                len(index_html_soup.select("#page-footer-hosting-name"))
                                > 0
                            ):
                                index_html_soup.select("#page-footer-hosting-name")[
                                    0
                                ].string = "GitHub Pages"
                            else:
                                logging.warning(
                        ...
                        try:
                            if (
                                ...
                                index_html_soup.select("#page-footer-source-code-link")[
                                    ...
                                    "href"
                                ] = "https://github.com/twfgcicdbot/TinyWeatherForecastGermanyScan/tree/gh-pages"
                            else:
                                logging.warning(
                        ...
                        try:
                            if (
                                ...
                                index_html_soup.select(
                                    ...
                                    "href"
                                ] = "https://github.com/twfgcicdbot/TinyWeatherForecastGermanyScan/blob/d19eb5eeeda3649ecd93a3b52f018878dd24ec81/LICENSE"
                            else:
                                logging.warning(
                        ...
                        try:
                            ...
                            if len(stars_count) > 0:
                                stars_count[0][
                                    "href"
                                ] = f"{gitlab_pages_url}stargazers.html"
                        ...
                        try:
                            ...
                            if len(watchers_count) > 0:
                                watchers_count[0][
                                    "href"
                                ] = f"{gitlab_pages_url}watchers.html"
                        ...
                        report_html_file = working_dir / "index.html"
                        try:
                            with open(report_html_file, "w+", encoding="utf-8") as fh:
                                fh.write(
                                    htmlmin.minify(
                                        report_file_html, remove_empty_space=True
                                    )
                                )
                        except Exception as error_msg:
                            logging.error(
                    ...
                    robots_p = working_dir / "robots.txt"
                    try:
                        robots_txt = """
User-agent: *
Allow: /
...
Sitemap: https://twfgcicdbot.github.io/TinyWeatherForecastGermanyScan/sitemap.xml
                        ...
                        with open(robots_p, "w+", encoding="utf-8") as fh:
                            fh.write(str(robots_txt).strip())
                    except Exception as error_msg:
                        logging.error(
                            f"failed to generate {robots_p} -> error: {error_msg}"
                        )
                    ...
                    sitemap_p = working_dir / "sitemap.xml"
                    try:
                        sitemapXML = (
                            """
 ...
 <url>
   <loc>https://twfgcicdbot.github.io/TinyWeatherForecastGermanyScan</loc>
    ...
    <priority>1.0</priority>
                        ...
                        with open(sitemap_p, "w+", encoding="utf-8") as fh:
                            fh.write(str(sitemapXML).strip())

                    except Exception as error_msg:
                        logging.error(
                            f"while generarting '{sitemap_p}' -> error: {error_msg}"
                        )

                except Exception as error_msg:
                    logging.error(
                        f"while processing '{apk_file_tmp}' -> error: {error_msg}"
                    )
    except Exception as error_msg:
